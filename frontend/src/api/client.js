...
export const auth = {
  ...
  users: () => get("/auth/users"),
    ...
    previewInvite: (token) => get(`/auth/invites/${encodeURIComponent(token)}`),
    acceptInvite: (token, payload) => post(`/auth/invites/${encodeURIComponent(token)}/accept`, payload),
    declineInvite: (token) => post(`/auth/invites/${encodeURIComponent(token)}/decline`),
