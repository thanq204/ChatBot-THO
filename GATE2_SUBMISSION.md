...
| Knowledge runtime | [backend/services/operations_store.py](backend/services/operations_store.py) | `list_knowledge`, `search_knowledge` | Đọc knowledge và embedding từ Supabase PostgreSQL + pgvector |
...
| Lấy context và moderation memory | [backend/services/operations_store.py](backend/services/operations_store.py) | `operations_messages`, `operations_moderation_marks`, `operations_moderation_embeddings` trên Supabase | Cung cấp lịch sử gần và case đã được người thật xác nhận |
...
- Runtime cần `FAQ_PG_DSN` trỏ tới Supabase. SQLite chỉ dùng cho test cô lập khi được truyền URL rõ ràng, không phải fallback production.
