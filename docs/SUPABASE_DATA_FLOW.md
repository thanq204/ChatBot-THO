...
| Member tag chatbot | Câu hỏi an toàn sau Rule và Moderation | Ghi mọi câu hỏi trước khi vào FAQ/RAG/LLM | `operations_faq_questions` | Dataset thống kê câu hỏi thật của member |
...
| Import tài liệu | CSV/JSON/JSONL/MD/TXT/DOCX/PDF tối đa 5 MB | Parse, semantic extractor tối đa 50 dòng, canonical validation | `operations_knowledge_imports`, `knowledge_import_raw`, `knowledge_normalized_records` | Nguồn truy vết đầy đủ từ raw đến document |
...
| LLM trả lời chung | Câu hỏi không cần tài liệu như tên bot/ngày giờ | Intent routing và LLM có giới hạn | Câu hỏi vẫn nằm trong `operations_faq_questions`; message ở `operations_messages` | Trả `[LLM]`, không giả citation RAG |
