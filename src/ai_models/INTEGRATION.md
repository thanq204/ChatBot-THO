...
4. Với câu hỏi tag bot hoặc chat riêng, chạy moderation trước. Gate 1 không đạt thì đọc context ở Gate 2; Gate 2 không đạt mới truy xuất case Admin/Mod ở Gate 3. Kết quả khác `allow` phải dừng trước FAQ/RAG/LLM.
...
8. Sau khi gửi câu trả lời cho member, lưu mỗi câu hỏi an toàn đúng một lần và gom semantic Top 10 trong background. Không ghi command hoặc câu đã bị moderation chặn.
9. Sau khi Admin/Mod quyết định một review, ghi moderation mark vào `operations_moderation_marks`, vector vào `operations_moderation_embeddings` trên Supabase.
...
Runtime giữ pool PostgreSQL mặc định `1..8` kết nối, cache embedding tối đa `128` câu và cache policy `15` giây. Các giới hạn này lần lượt cấu hình bằng `POSTGRES_POOL_MIN_SIZE`, `POSTGRES_POOL_MAX_SIZE`, `SEMANTIC_EMBEDDING_CACHE_SIZE` và `MODERATION_POLICY_CACHE_SECONDS`.
...
RAG_SEMANTIC_ONLY_MIN_SCORE=0.62
