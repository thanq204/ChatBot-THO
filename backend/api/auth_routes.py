...
async def me(user: UserPublic = Depends(current_user)) -> UserPublic: return user

@router.get("/users", response_model=list[UserPublic])
async def users(_: UserPublic = Depends(require_roles("admin"))) -> list[UserPublic]: return get_auth_store().list_users()
...
async def update_status(user_id: str, payload: UserStatusUpdateRequest, _: UserPublic = Depends(require_roles("admin"))) -> UserPublic:
    from uuid import UUID
...
async def delete_user(user_id: str, admin: UserPublic = Depends(require_roles("admin"))) -> None:
    from uuid import UUID
    try:
        target = UUID(user_id)
